...
use testapi;
#use utils;
use alp_workloads::kvm_workload_utils qw(set_kvm_image clean_and_resetup_kvm_container collect_kvm_container_setup_logs);

sub run {
    my $self = shift;

    if (get_var('KVM_WORKLOAD_IMAGE', '')) {
        set_kvm_image(get_var('KVM_WORKLOAD_IMAGE'));
    }
